...
- Q: How should the user move between setup (text input) and reading (word display) modes? → A: Explicit buttons: Read and Edit Text.
...
- **FR-018**: System MUST provide explicit mode-transition controls: a **Read** action to move from text input to reading display and an **Edit Text** action to return to input mode.
